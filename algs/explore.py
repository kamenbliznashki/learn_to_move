...
class SPEnsemble:
    ...
    def reward_fn(self, obs, actions, pred_next_obs):
        ...
        x_vtgt_onehot, _, goal_distance = tf.split(
                vtgt, [(self.v_tgt_field_size - 1)//2, (self.v_tgt_field_size - 1)//2, 1], axis=-1)  # out (B, x_vtgt_onehot), (B, y_vtgt_onehot) and (B, 1)
        height, pitch, roll, dx, dy, dz, dpitch, droll, dyaw = tf.split(pred_next_obs[...,self.v_tgt_field_size: self.v_tgt_field_size+9], 9, -1)
        rewards = RewardAugEnv.compute_rewards(x_vtgt_onehot, goal_distance, height, pitch, roll, dx, dy, dz, dpitch, droll, dyaw, None, None, None, None, None, None)
